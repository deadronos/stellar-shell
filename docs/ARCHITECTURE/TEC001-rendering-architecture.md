...
3.  **Presentation Layer (R3F)**:
    ...
    - `RenderChunk` (`src/render/RenderChunk.tsx`): Observes `entity.meshData` changes via `useEffect`, applies them to a stable `THREE.BufferGeometry` via `MeshUpdater.updateGeometry()`, and disposes that geometry on unmount.
    - `CompletedSectionRenderer` (`src/render/CompletedSectionRenderer.tsx`): Same geometry update/disposal path, but shares a single module-level `THREE.MeshStandardMaterial` for reduced draw calls.
