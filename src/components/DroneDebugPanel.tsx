    ...
    <div className="fixed top-20 right-6 z-40 w-80 bg-black/80 backdrop-blur border border-white/10 rounded-lg p-4 font-mono text-xs max-h-[80vh] overflow-y-auto pointer-events-auto">
      ...
      <div className="space-y-2">
        {drones.map((drone, index) => (
          <div
            key={drone.id ?? `${drone.state ?? 'UNKNOWN'}-${index}`}
            className="border border-white/5 bg-white/5 p-2 rounded hover:bg-white/10 transition-colors"
          >
            <div className="flex justify-between items-center mb-1">
              <span className="text-gray-400">ID: {drone.id ?? 'N/A'}</span>
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold ${getStateColor(drone.state)}`}
              >
                {drone.state}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] text-gray-500">
              <div>Pos: {formatVec3(drone.position)}</div>
              <div>Target: {drone.target ? formatVec3(drone.target) : 'NONE'}</div>
            </div>
            ...
            {drone.carryingType && (
              <div className="mt-1 text-[10px] text-purple-400">Carrying: {drone.carryingType}</div>
            )}
          </div>
        ))}
        {drones.length === 0 && (
...
const formatVec3 = (v: { x: number; y: number; z: number }) =>
  `(${v.x.toFixed(0)}, ${v.y.toFixed(0)}, ${v.z.toFixed(0)})`;
