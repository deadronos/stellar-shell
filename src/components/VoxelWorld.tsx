...
const ChunkRenderer: React.FC<{ chunkKey: string, engine: BvxEngine }> = ({ chunkKey, engine }) => {
  ...
  useFrame((state) => {
      if (state.clock.elapsedTime % 0.5 < 0.1) { // Check periodically
  ...
  return (
    <mesh ref={meshRef} position={[chunk.position.x * CHUNK_SIZE, chunk.position.y * CHUNK_SIZE, chunk.position.z * CHUNK_SIZE]}>
      <bufferGeometry />
